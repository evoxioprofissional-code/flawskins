...
export function PrecoBuff({ preco, dados }: { preco: number; dados: PrecoRef }) {
  if (!dados?.buff) return null;

  const diff = ((preco - dados.buff) / dados.buff) * 100;
  const abaixo = diff <= -2;
  const acima = diff >= 2;
  ...
  return (
    <div className="mt-3 flex items-center gap-2 rounded-lg border border-white/10 bg-neutral-950 px-3 py-2 text-sm">
      <span className="text-zinc-400">
        Buff163:{" "}
        <span className="font-semibold text-zinc-100">{formatBRL(dados.buff)}</span>
      </span>
      <span
        className={
